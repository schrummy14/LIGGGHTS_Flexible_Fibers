...
TANGENTIAL_MODEL(TANGENTIAL_JKR_TAN,jkr_tan,11)
#else
#ifndef TANGENTIAL_MODEL_JKR_TAN_H_
#define TANGENTIAL_MODEL_JKR_TAN_H_
...
{
  template<>
  class TangentialModel<TANGENTIAL_JKR_TAN> : public TangentialModelBase
  {
    double ** coeffFrict;
    int history_offset;
  ...
  public:
    ...
    inline void connectToProperties(PropertyRegistry & registry)
    {
        ...
        registry.connect("coeffFrict", coeffFrict,"tangential_model jkr_tan");
    ...
    inline void surfacesIntersect(const SurfacesIntersectData & sidata, ForceData & i_forces, ForceData & j_forces)
    {
        ...
        //If no work of adhesion then same as tangential_model_history
        double Ft_friction,Ft;
        if (workOfAdhesion[sidata.itype][sidata.jtype] == 0.0)
        {
            Ft = sidata.kn*sidata.deltan;
        }
        else
        {
