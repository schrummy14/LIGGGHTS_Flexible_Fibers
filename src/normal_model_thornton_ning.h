    ...
    namespace ContactModels
    {
        ...
        public:
            ...
            inline double calculate_elastic_force_differential(double a, double E, double fl, double fc)
            {
                ...
                if (fc != 0)
                {
                    const double up = 3 * sqrt(fl) - 3 * sqrt(fc);
                    const double down = 3 * sqrt(fl) - sqrt(fc);
                    df = 2 * E * a * up / down;
                }
                else
                {
            ...
            inline double calculate_plastic_force_differential(double a_yield, double E, double fl, double fc, double yield_stress, double R)
            {
                ...
                if (fc != 0)
                {
                    const double up = 3 * M_PI * R * yield_stress * sqrt(fl) - 2 * a_yield * E * sqrt(fc);
                    const double down = 3 * sqrt(fl) - sqrt(fc);
                    df = up / down;
                }
                else
            ...
            {
                const double a_over_ac = a / ac;
                const double daf = pow(3, (1. / 3.)) * pow(a_over_ac, 2) * (1 - (4. / 3.) * pow(a_over_ac, -(3. / 2.)));
            ...
            {
                const double fl_r = calculate_fl(force_old, fc, adhesion_flag);
                const double a = calculate_a(fl_r, E, rp, delta, gamma_s);
                ...
                const double df = calculate_elastic_force_differential(a, E, fl_r, fc) * dn;
...
#               endif
                ...
                if (yield_stress < limit_yield_stress)
                    plastic_from_start = 1;

                if (yield_stress <= 0)
                    error->all(FLERR, "Invalid yield stress, please check surface energy and yield ratio!");
                ...
                const double k_t = 8. * Geff[itype][jtype] * sqrtval;
                const double gammat = 2. * sqrtFiveOverSix * betaeff[itype][jtype] * sqrt(k_t * meff);
