...
#include <stdlib.h>
#include <string.h>
#include "fix_nve_limit_sphere.h"
#include "atom.h"
#include "force.h"
#include "update.h"
#include "respa.h"
...
FixNVELimitSphere::FixNVELimitSphere(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
...
void FixNVELimitSphere::initial_integrate(int vflag)
{
  double dtfm,vsq,scale,rsq,osq,dtirotate,dtfrotate; 
...
void FixNVELimitSphere::final_integrate()
{
  double dtfm,vsq,scale,rsq,osq,dtirotate,dtfrotate; 
...
void FixNVELimitSphere::initial_integrate_respa(int vflag, int ilevel, int iloop)
{
...
void FixNVELimitSphere::final_integrate_respa(int ilevel, int iloop)
{
  dtf = 0.5 * step_respa[ilevel] * force->ftm2v;
  final_integrate();
}
...
void FixNVELimitSphere::reset_dt()
{
  dtv = update->dt;
  dtf = 0.5 * update->dt * force->ftm2v;
  vlimitsq = (xlimit/dtv) * (xlimit/dtv);
...
double FixNVELimitSphere::compute_scalar()
{
  double one = ncount;
  double all;
